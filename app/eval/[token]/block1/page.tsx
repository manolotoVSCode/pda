...
import { MAIN_WORDS } from '@/lib/instrument/words'
...
export default function Block1Page({ params }: Props) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true)
    setError(null)
    const res = await fetch(`/api/assessments/${params.token}/block1`, {
      method: 'POST',
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-600 bg-slate-100 rounded-xl p-4 leading-relaxed">
        Lee todas las palabras y selecciona exactamente <strong>{LIMIT}</strong> que{' '}
        <strong>mejor te describan</strong> según cómo crees que los demás te perciben.
